  ...
  return (
    <div className="watch-alert" role="alert" onClick={() => { select(aircraft.hex); setWatchHit(null); }}>
      ...
      <div className="watch-alert-text">
        <div className="watch-alert-title">
          {watch.raw_input.toUpperCase()} is in view!
        </div>
        <div className="watch-alert-sub">
          {[operator, type, origin && dest ? `${origin} → ${dest}` : null, aircraft.distNm != null ? `${aircraft.distNm} nm` : null]
            .filter(Boolean).join(" · ")}
        </div>
      </div>
      <button
